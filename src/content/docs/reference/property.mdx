...
### config.texture （只支持 3D模式）

<br/>

<Tag color="blue">类型: object</Tag>
<Tag color="pink">说明: 地球/地图的贴图效果,如果使用 mixed 则是 json 地图文件和贴图混合使用</Tag>
<Tag color="purple">默认</Tag>
``` json
// 混合策略的目的是希望关于部分地图色块复杂定制化（目前组件做不到的复杂样式），比如说希望给某个国家进行设置一个复杂的样式这种样式只能通过贴图来满足，其他的地区样式依旧采用 js 绘制。
...
{
  path:'path/to/image.png',  //图片地址 格式可以是img图片 或者svg,
  mixed: false, //是否混合地图和地球贴图 如果为true记得注册 json 地图文件
}
```
<br/>
...
<Alert color="primary">
    ...
    | "MeshNormalMaterial"
    <br/>
    想要细致了解材质请参考<a href="https://threejs.org/docs/#api/zh/materials/MeshBasicMaterial">文档</a>
</Alert>
### config.mapStyle
<br/>
<Tag color="blue">类型: object</Tag>
