...
<div>
  type:
  <Tag color="cyan">flyLine</Tag>
  <span style="margin-right:8px;">|</span>
  <Tag color="cyan">point</Tag>
  <span style="margin-right:8px;">|</span>
  <Tag color="cyan">road</Tag>
  <span style="margin-right:8px;">|</span>
  <Tag color="cyan">wall</Tag>
  <span style="margin-right:8px;">|</span>
  <Tag color="cyan">mapStreamLine</Tag>
  <span style="margin-right:8px;">|</span>
  <Tag color="cyan">bar</Tag>
  <span style="margin-right:8px;">|</span>
  <Tag color="cyan">textMark</Tag>
</div>
...
pathStyle 请参考 <a style="color:" href="javascript:void(0);" data-href="/reference/property/#configpathstyle"><span style="color:#1890ff;">config.pathStyle</span></a>

flyLineStyle 请参考 <a style="color:" href="javascript:void(0);" data-href="/reference/property/#configflylinestyle"><span style="color:#1890ff;">config.flyLineStyle</span></a>
...
const barData = [
  {
    ...
    value: 100, //柱状图高度
    style:barStyle //字段解释参考下方链接
    ...userData // 其他自定义字段
  }
]
chart.addData('bar',pointData)
```
<Alert color="primary">
barStyle请参考 <a style="color:" href="javascript:void(0);" data-href="/reference/property/#configbarstyle"><span style="color:#1890ff;">config.scatterStyle</span></a>
...
<br/>
<Tag color="red">type=textMark</Tag>
``` javascript
chart.setData("textMark", [
    {
      text: "中国",
      position: {
        lon: 104.195397,
...
```
<Alert color="warning">
id字段：from和to里的id最终会拼接在一起<span style="color:#1890ff;">id=${lon}-${lat}-${text}</span> ,如果没有传id 则会根据经纬度拼接 拼接逻辑为：
<span style="color:#1890ff;">id =$\{from.lon\}$\{from.lat\}-$\{to.lon\}$\{to.lat\}</span> 此id被用于remove方法
</Alert>
...
chart.remove("bar", "removeAll"); //bar 目前只能removeAll 不支持单个移除
```
